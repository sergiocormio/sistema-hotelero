 ...
 * 
 * 	Room 1: 	XXXXXX        XXXXXXXXX        XXXX 
 *  Room 2:           XXXXXXXX         XXXXXXXX    XXX                      
 *  Room 3:                            XXXXXXXXXXXXXXX
 * 
 * Result: 
 * 
 * 	NoAvailableAlternativesException: No available alternatives for date: 10-7-2012
 *  cause: room 3 is the one that survives all roomChanges until the last day, when the number 
 *  	   of room changes becomes invalid (roomChanges have to be less than 3)

 * @author fede
 *
 */
public class ScenarioBuilder_RoomChangesExceeded_SameRoomType {
	...
	public ScenarioBuilder_RoomChangesExceeded_SameRoomType() throws DaoException {
	...
	private void fillOccupationsForAllRooms() throws DaoException {
		...
		dateFrom = new GregorianCalendar(2012, 7, 5).getTime();
		dateTo = new GregorianCalendar(2012, 7, 6).getTime();
		
		fillOccupations(dateFrom, dateTo, roomId, room);
		
		dateFrom = new GregorianCalendar(2012, 7, 9).getTime();
		dateTo = new GregorianCalendar(2012, 7, 9).getTime();
		...
		dateFrom = new GregorianCalendar(2012, 7, 3).getTime();
		dateTo = new GregorianCalendar(2012, 7, 4).getTime();
		
		fillOccupations(dateFrom, dateTo, roomId, room);

		dateFrom = new GregorianCalendar(2012, 7, 7).getTime();
		dateTo = new GregorianCalendar(2012, 7, 8).getTime();
